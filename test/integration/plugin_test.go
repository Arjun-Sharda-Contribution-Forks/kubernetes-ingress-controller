...
func TestPluginOrdering(t *testing.T) {
	...
	defer func() {
		t.Log("cleaning up plugins")
		if err := c.ConfigurationV1().KongPlugins(ns.Name).Delete(ctx, termplugin.Name, metav1.DeleteOptions{}); err != nil {
			if !errors.IsNotFound(err) {
				assert.NoError(t, err)
			}
		}
		if err := c.ConfigurationV1().KongPlugins(ns.Name).Delete(ctx, authplugin.Name, metav1.DeleteOptions{}); err != nil {
			if !errors.IsNotFound(err) {
				assert.NoError(t, err)
			}
		}
